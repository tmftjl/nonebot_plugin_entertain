...
P = Plugin(name="help", display_name="帮助", enabled=True, level=PermLevel.LOW, scene=PermScene.ALL)

# 触发：<关键词>(帮助|菜单|功能)
matcher = P.on_regex(r"^(?:#|/)?(.*?)\s*(帮助|菜单|功能)$", name="help",display_name="帮助", priority=5, block=True, enabled=True, level=PermLevel.LOW, scene=PermScene.ALL)
