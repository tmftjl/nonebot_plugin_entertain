...
function switchTab(tab){
  ...
  else if(tab==='all-config') loadAllPluginConfigs();
}
...
// 配置字段定义
const CONFIG_FIELDS = [
  { key: 'member_renewal_timezone', label: '⏰ 时区设置', type: 'text', description: '插件运行的时区（如：Asia/Shanghai）' },
  { key: 'member_renewal_enable_scheduler', label: '📅 启用定时任务', type: 'boolean', description: '是否启用自动检查会员到期' },
  { key: 'member_renewal_schedule_hour', label: '🕐 定时小时', type: 'number', description: '每天执行检查的小时(0-23)' },
  { key: 'member_renewal_schedule_minute', label: '🕑 定时分钟', type: 'number', description: '每天执行检查的分钟(0-59)' },
  { key: 'member_renewal_schedule_second', label: '🕒 定时秒', type: 'number', description: '每天执行检查的秒(0-59)' },
  { key: 'member_renewal_reminder_days_before', label: '⏳ 提前提醒天数', type: 'number', description: '会员到期前多少天开始提醒' },
  { key: 'member_renewal_daily_remind_once', label: '🔔 每日仅提醒一次', type: 'boolean', description: '每天是否只发送一次提醒' },
  { key: 'member_renewal_contact_suffix', label: '📞 联系方式后缀', type: 'textarea', description: '提醒消息末尾添加的联系信息' },
  { key: 'member_renewal_remind_template', label: '📝 提醒模板', type: 'textarea', description: '提醒消息模板，可用变量：{days}、{expiry}' },
  { key: 'member_renewal_soon_threshold_days', label: '⚠️ 即将到期阈值', type: 'number', description: '多少天内算即将到期' },
  { key: 'member_renewal_auto_leave_on_expire', label: '🚪 到期自动离群', type: 'boolean', description: '会员到期后是否自动离开群组' },
  { key: 'member_renewal_leave_mode', label: '👋 离群模式', type: 'select', options: [{ value: 'leave', label: '主动退出' }, { value: 'kick', label: '踢出群组' }], description: '到期后的离群方式' },
  { key: 'member_renewal_default_bot_id', label: '🤖 默认Bot ID', type: 'text', description: '默认使用的机器人ID' },
  { key: 'member_renewal_console_enable', label: '💻 启用控制台', type: 'boolean', description: '是否启用Web控制台' },
  { key: 'member_renewal_stats_api_url', label: '📊 统计API地址', type: 'text', description: 'Web控制台统计API的基础URL' },
  { key: 'member_renewal_code_prefix', label: '🎫 续费码前缀', type: 'text', description: '生成续费码的前缀' },
  { key: 'member_renewal_code_random_len', label: '🔢 续费码随机长度', type: 'number', description: '续费码随机部分的长度' },
  { key: 'member_renewal_code_expire_days', label: '⏰ 续费码有效期', type: 'number', description: '续费码有效天数(0表示永久)' },
  { key: 'member_renewal_code_max_use', label: '♻️ 续费码最大使用次数', type: 'number', description: '每个续费码可使用的最大次数' }
];

function renderConfigVisualEditor(cfg) {
  const container = $('#config-visual-editor');
  if (!container) return;
  container.innerHTML = '';
  ...
  CONFIG_FIELDS.forEach(field => {
    ...
    if (field.type === 'boolean') {
      controlHTML = `<label class="toggle-switch"><input type="checkbox" ${value ? 'checked' : ''} data-key="${field.key}"><span class="toggle-slider"></span></label>`;
    } else if (field.type === 'number') {
      controlHTML = `<input type="number" value="${value || 0}" data-key="${field.key}">`;
    } else if (field.type === 'textarea') {
      controlHTML = `<textarea data-key="${field.key}" rows="3">${value || ''}</textarea>`;
    } else if (field.type === 'select' && field.options) {
      const opts = field.options.map(o => `<option value="${o.value}" ${value === o.value ? 'selected' : ''}>${o.label}</option>`).join('');
      controlHTML = `<select data-key="${field.key}">${opts}</select>`;
    } else {
      controlHTML = `<input type="text" value="${value || ''}" data-key="${field.key}">`;
    }
    
    item.innerHTML = `
      <div class="config-item-label">${field.label}</div>
      <div class="config-item-description">${field.description}</div>
      <div class="config-item-control">${controlHTML}</div>
    `;
  ...
  container.querySelectorAll('input, textarea, select').forEach(input => {
    input.addEventListener('change', () => {
      ...
      else if (input.type === 'number') val = parseInt(input.value) || 0;
...
async function saveConfig() {
  try {
    showLoading(true);
    await apiCall('/config', { method: 'PUT', body: JSON.stringify(state.config) });
...
function openConfigJsonModal() {
  const modal = $('#config-json-modal');
  const editor = $('#config-json-editor');
  const title = $('#config-modal-title');
  if (modal && editor) {
    if (title) title.textContent = '📝 编辑全局配置JSON';
    editor.value = JSON.stringify(state.config || {}, null, 2);
    editor.dataset.plugin = '';
...
async function saveConfigJson() {
  try {
    ...
    const plugin = editor?.dataset.plugin || '';
    ...
    let url = '/config';
    if (plugin && filename) {
      url = `/plugin-config/${plugin}/${filename}`;
    }
    
    await apiCall(url, { method: 'PUT', body: JSON.stringify(cfg) });
    
    if (plugin && filename) {
      showToast(`${plugin} - ${filename} 配置已保存`, 'success');
      await loadAllPluginConfigs();
    } else {
      state.config = cfg;
      renderConfigVisualEditor(cfg);
      showToast('全局配置已保存', 'success');
    }
    
    closeConfigJsonModal();
  } catch (e) {
    showToast('JSON格式错误或保存失败: ' + (e && e.message ? e.message : e), 'error');
  } finally {
    showLoading(false);
  }
}
...
// ==================== 所有插件配置管理 ====================
let allPluginConfigs = {};

async function loadAllPluginConfigs() {
  try {
    showLoading(true);
    const configs = await apiCall('/all-configs');
    allPluginConfigs = configs;
    renderAllPluginConfigs(configs);
  } catch (e) {
    showToast('加载插件配置失败: ' + (e && e.message ? e.message : e), 'error');
  } finally {
    showLoading(false);
  }
}

function renderAllPluginConfigs(configs) {
  const container = $('#all-plugins-container');
  if (!container) return;
  
  const plugins = Object.keys(configs).sort();
  if (!plugins.length) {
    container.innerHTML = '<div class="empty-state" style="padding:60px 20px;"><div style="font-size:48px;margin-bottom:16px;">📦</div><div>暂无插件配置</div></div>';
    return;
  }
  
  // Render as a grid of plugin cards
  const cards = plugins.map(plugin => {
    const files = configs[plugin];
    const fileNames = Object.keys(files);
    const fileCount = fileNames.length;
    
    return `
      <div class="plugin-config-card" data-plugin="${plugin}">
        <div class="plugin-config-card-icon">🔧</div>
        <div class="plugin-config-card-info">
          <div class="plugin-config-card-title">${plugin}</div>
          <div class="plugin-config-card-meta">${fileCount} 个配置文件</div>
        </div>
        <div class="plugin-config-card-actions">
          ${fileNames.map(fname => `
            <button class="btn btn-primary btn-sm plugin-config-open-btn"
                    data-plugin="${plugin}"
                    data-file="${fname}">
              ${fileNames.length > 1 ? fname : '查看配置'}
            </button>
          `).join('')}
        </div>
      </div>
  ...
  container.innerHTML = `<div class="plugin-config-grid">${cards}</div>`;
  
  // Bind click events to open modal
  container.querySelectorAll('.plugin-config-open-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const plugin = btn.dataset.plugin;
      const file = btn.dataset.file;
      openPluginConfigModal(plugin, file, configs[plugin][file]);
    });
  });
}

function renderPluginConfigEditor(plugin, filename, config) {
  const container = $(`#editor-${plugin}-${filename}`);
  if (!container) return;
  
  const entries = Object.entries(config);
  if (!entries.length) {
    container.innerHTML = '<div class="empty-state">该配置文件为空</div>';
    return;
  }
  ...
  container.innerHTML = entries.map(([key, value]) => {
    ...
    if (type === 'boolean') {
      controlHTML = `
        <label class="toggle-switch">
          <input type="checkbox" ${value ? 'checked' : ''}
                 data-plugin="${plugin}" data-file="${filename}" data-key="${key}">
          <span class="toggle-slider"></span>
        </label>
      `;
    } else if (type === 'number') {
      controlHTML = `<input type="number" value="${value}"
                            data-plugin="${plugin}" data-file="${filename}" data-key="${key}">`;
    } else if (type === 'string' && value.length > 50) {
      controlHTML = `<textarea data-plugin="${plugin}" data-file="${filename}" data-key="${key}"
                               rows="3">${value}</textarea>`;
    } else if (Array.isArray(value)) {
      controlHTML = `<input type="text" value="${value.join(', ')}"
                            data-plugin="${plugin}" data-file="${filename}" data-key="${key}"
      ...
      controlHTML = `<input type="text" value="${value || ''}"
                            data-plugin="${plugin}" data-file="${filename}" data-key="${key}">`;
    }
    
    return `
      <div class="config-item">
        <div class="config-item-label">${key}</div>
        <div class="config-item-description">类型: ${Array.isArray(value) ? 'array' : type}</div>
        <div class="config-item-control">${controlHTML}</div>
      </div>
    `;
  }).join('');
  ...
  container.querySelectorAll('input, textarea').forEach(input => {
    input.addEventListener('change', () => {
      ...
      } else if (input.placeholder && input.placeholder.includes('逗号')) {
        val = input.value.split(',').map(v => v.trim()).filter(Boolean);
      } else {
        try {
      ...
      if (!allPluginConfigs[plugin]) allPluginConfigs[plugin] = {};
      if (!allPluginConfigs[plugin][file]) allPluginConfigs[plugin][file] = {};
      allPluginConfigs[plugin][file][key] = val;
    });
  });
}

function togglePluginConfig(plugin) {
  const body = $(`#body-${plugin}`);
  const toggle = $(`#toggle-${plugin}`);
...
function switchConfigFile(plugin, filename) {
  const tabsContainer = $(`#tabs-${plugin}`);
  if (!tabsContainer) return;
  
  tabsContainer.querySelectorAll('.config-file-tab').forEach(tab => {
    tab.classList.toggle('active', tab.dataset.file === filename);
  });
  
  const panel = tabsContainer.closest('.plugin-config-panel');
  panel.querySelectorAll('.config-file-content').forEach(content => {
    content.classList.toggle('active', content.id === `content-${plugin}-${filename}`);
  });
}

async function savePluginConfig(plugin, filename) {
  try {
    showLoading(true);
    const config = allPluginConfigs[plugin]?.[filename] || {};
    await apiCall(`/plugin-config/${plugin}/${filename}`, {
      method: 'PUT',
...
function viewConfigJSON(plugin, filename) {
  const modal = $('#config-json-modal');
  const editor = $('#config-json-editor');
  const title = $('#config-modal-title');
  ...
  if (title) title.textContent = `📝 ${plugin} - ${filename}`;
  editor.value = JSON.stringify(config, null, 2);
  editor.dataset.plugin = plugin;
  editor.dataset.filename = filename;
  modal.classList.remove('hidden');
}

function openPluginConfigModal(plugin, filename, config) {
  // Create modal dynamically
  const existingModal = $('#plugin-config-edit-modal');
  if (existingModal) existingModal.remove();
  
  const modal = document.createElement('div');
  modal.id = 'plugin-config-edit-modal';
  modal.className = 'modal';
  modal.innerHTML = `
    <div class="modal-dialog" style="max-width: 1200px;">
      <div class="modal-header">
        <h3 class="panel-title">🔧 ${plugin} - ${filename}</h3>
        <button class="modal-close" onclick="document.getElementById('plugin-config-edit-modal').remove()">✖</button>
      </div>
      <div class="modal-body">
        <div class="config-visual-editor" id="plugin-config-editor-content"></div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-secondary" id="plugin-config-view-json-btn">📝 查看JSON</button>
        <button class="btn btn-secondary" onclick="document.getElementById('plugin-config-edit-modal').remove()">❌ 取消</button>
        <button class="btn btn-primary" id="plugin-config-save-modal-btn">💾 保存配置</button>
      </div>
    </div>
  `;
  ...
  // Render config editor inside modal
  const editorContainer = $('#plugin-config-editor-content');
  if (editorContainer) {
    renderPluginConfigEditorInModal(plugin, filename, config, editorContainer);
  }
  
  // Bind modal buttons
  $('#plugin-config-view-json-btn')?.addEventListener('click', () => {
    viewConfigJSON(plugin, filename);
  });
  
  $('#plugin-config-save-modal-btn')?.addEventListener('click', async () => {
    await savePluginConfigFromModal(plugin, filename);
  });
...
function renderPluginConfigEditorInModal(plugin, filename, config, container) {
  const entries = Object.entries(config);
  if (!entries.length) {
    container.innerHTML = '<div class="empty-state">该配置文件为空</div>';
    return;
...
async function savePluginConfigFromModal(plugin, filename) {
  try {
    showLoading(true);
    const config = allPluginConfigs[plugin]?.[filename] || {};
    await apiCall(`/plugin-config/${plugin}/${filename}`, {
    ...
    showToast(`${plugin} - ${filename} 配置已保存`, 'success');
    $('#plugin-config-edit-modal')?.remove();
    await loadAllPluginConfigs();
  } catch (e) {
    showToast('保存失败: ' + (e && e.message ? e.message : e), 'error');
  } finally {
...
// 导出函数供HTML使用
window.togglePluginConfig = togglePluginConfig;
window.switchConfigFile = switchConfigFile;
window.savePluginConfig = savePluginConfig;
window.viewConfigJSON = viewConfigJSON;
...
function bindEvents(){
  ...
  $('#open-config-json-btn')?.addEventListener('click', openConfigJsonModal);
  $('#config-json-close')?.addEventListener('click', closeConfigJsonModal);
  $('#config-json-cancel')?.addEventListener('click', closeConfigJsonModal);
  $('#config-json-save')?.addEventListener('click', saveConfigJson);
