...
# Regex definitions
search_matcher = on_regex(
    r"^#?点歌(?:(qq|酷狗|网易云|wyy|kugou|netease))?\s*(.*)$",priority=5,flags=0,
)

select_matcher = on_regex(r"^#?(\d+)$", flags=0)

# Centralized permission control
try:
    search_matcher.permission = permission_for("musicshare")
    select_matcher.permission = permission_for("musicshare")
except Exception:
    pass
...
async def _search_songs(platform: Platform, keyword: str) -> List[Song]:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        if platform == "kugou":
            ...
            results: List[Song] = []
            for item in infos:
                name = item.get("songname") or "鏈煡姝屾洸"
                artist = item.get("singername") or "鏈煡鑹轰汉"
        ...
        if platform == "qq":
            ...
            for item in items:
                ...
                artist = item.get("singers") or "鏈煡鑹轰汉"
                mid = item.get("mid")
        ...
        for s in songs:
            sid = str(s.get("id"))
            name = s.get("name") or "鏈煡姝屾洸"
            artists = s.get("artists") or []
            artist = artists[0].get("name") if artists else "鏈煡鑹轰汉"
            cover = None
...
async def _download_audio_via_ffmpeg(url: str, name_hint: str) -> Optional[Path]:
    """
    在宿主机上下载/转换音频，但返回对 NapCat 容器内部有效的路径。
    """
    ffmpeg = _ffmpeg_path()
    if not ffmpeg:
    ...
    try:
        ...
        if proc.returncode == 0 and host_out_path.exists() and host_out_path.stat().st_size > 1024:
            ...
            logger.success(f"FFmpeg created file {host_out_path}, returning container path {container_return_path}")
            ...
            if not host_out_path.exists():
                ...
                logger.error(f"Output file on host is too small ({host_out_path.stat().st_size} bytes).")
            return None
...
def _make_song_list_image_grid(platform: Platform, keyword: str, songs: List[Song]) -> bytes:
    ...
    title = f"为你从【{_platform_name_human(platform)}】找到以下歌曲"
    subtitle = f"鍏抽敭璇嶏細{keyword}"
...
async def _(matcher: Matcher, event: MessageEvent, groups: Tuple[Optional[str], str] = RegexGroup()) -> None:
    ...
    if not songs:
        await matcher.finish(
            f"在【{_platform_name_human(platform)}】中没有找到与“{keyword}”相关的歌曲。"
        )
        return
...
async def _(matcher: Matcher, event: MessageEvent, groups: Tuple[str] = RegexGroup()) -> None:
    ...
    # 优先尝试发送 URL 卡片，不行再自行处理发送（本地转码）
    audio_url = await _resolve_audio_url(platform, song)

    # 本地转码并发送语音（避免捕获 finish）
    if audio_url:
        try:
