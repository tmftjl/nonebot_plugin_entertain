...
async def _(matcher: Matcher, event: MessageEvent):
    ...
    await matcher.finish(Message(f"已生成续费码（仅可使用一次）：{code}\n请将其发送到需要开通/续费的群聊中（首次开通也使用此码）"))
...
try:
    ...
    if bool(cfg.get("member_renewal_enable_scheduler", True)):
        async def _job():
            try:
                r, l = await _check_and_process()
                logger.info(f"[membership] check finished, reminded={r}, left={l}")
            except Exception as e:
                logger.exception(f"[membership] check failed: {e}")
